...
public class EhcacheMessageTrackerMessage extends EhcacheSyncMessage {
  ...
  private final Map<ClientSourceId, Map<Long, EhcacheEntityResponse>> trackedMessages;

  public EhcacheMessageTrackerMessage(Map<ClientSourceId, Map<Long, EhcacheEntityResponse>> trackedMessages) {
  ...
  public EhcacheMessageTrackerMessage(OOOMessageHandler<EhcacheEntityMessage, EhcacheEntityResponse> messageHandler) {
    ...
    clientSourceIds.forEach(id -> {
      trackedMessages.put(id, new HashMap<>(messageHandler.getTrackedResponses(id))); // clone the map to prevent referencing the message handler one
    });
  }
  ...
  public Map<ClientSourceId, Map<Long, EhcacheEntityResponse>> getTrackedMessages() {
