...
public class EhcacheMessageTrackerMessageTest {
  ...
  private ClientSourceIdImpl id1 = new ClientSourceIdImpl(1);
  private ClientSourceIdImpl id2 = new ClientSourceIdImpl(2);
  ...
  public void getTrackedMessages() throws Exception {
    Map<ClientSourceId, Map<Long, EhcacheEntityResponse>> result = message.getTrackedMessages();
    assertThat(result).containsKeys(id1, id2);

    assertThat(result.get(id1)).contains(entry(3L, r3), entry(4L, r4));
    assertThat(result.get(id2)).contains(entry(5L, r5));
  }
